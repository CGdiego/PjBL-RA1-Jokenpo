...
# Modo Battle Royale
    ...
    while repeatF == "s":
        ...
        quantJog = str(quantJog)

        while not quantJog.isdigit() or int(quantJog) <= 1:
            quantJog = input("Insira a quantidade de jogadores: ")
            if not quantJog.isdigit() or int(quantJog) <= 1:
                print("\nNão é uma possibilidade, insira novamente.\n")

        quantJog = int(quantJog)
        ...
        if quantJog > 2:
            for i in range(1, quantJog+1):
                print(f"{i}ª pessoa. Outra(s) {quantJog-i} restante(s).")
